...
tags:
  - golang
---
很多年前刚开始接触golang 时，在极客时间学习的《go语言核心36讲》做的笔记。现在看来记得很一般。
...
atomic.Value 类型是开箱即用的，我们声明一个该类型的变量（以下简称原子变量）之后就可以直接使用了。这个类型使用起来很简单，它只有两个指针方法：Store和Load。不过，虽然简单，但还是有一些值得注意的地方的。
